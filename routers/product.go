...
func InsertProduct(body string, User string) (int, string) { //Recibe estos dos parámetros
	...
	if len(t.ProdTitle) == 0 {
		//Agregamos esta validación porque el unico campo que no es permitido que se guarde como null de la tabla product es el nombre(ProdTitle)
		return 400, "Debe especificar el Nombre (Title) del producto" + err.Error()
	}
	...
	if err2 != nil {
		return 400, "Ocurrió un error al intentar realizar el registro del producto" + err.Error()
	}
