...
func InsertOrder(o models.Orders) (int64, error) {
	...
	for _, od := range o.OrderDetails { //Para cada uno inserts de los items de la OrderDetail
...
func SelectOrders(user string, fechaDesde string, fechaHasta string, page int, orderId int) ([]models.Orders, error) {
	...
	if orderId > 0 {
		...
		if len(fechaHasta) == 10 { //10 es la cantidad de caracteres que tiene la fecha para ver si la fecha vino con la Hora o sin la Hora
			fechaHasta += " 23:59:59"
		}

		var where string
		var whereUser string = " Order_UserUUID = '" + user + "'"
		if len(fechaDesde) > 0 && len(fechaHasta) > 0 { //Si cualquiera viene vacia no filtra por fecha
