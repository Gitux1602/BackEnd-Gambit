...
func SelectUser(UserId string) (models.User, error) {
	...
	rows.Scan(&User.UserUUID, &User.UserEmail, &firstName, &lastName, &User.UserStatus, &User.UserDateAdd, &dateUpg) //Lo enviamos al modelo
...
func SelectUsers(Page int) (models.ListUsers, error) {
	...
	User := []models.User{} //Se ponen las llaves al final para que lo devuelva vacio
